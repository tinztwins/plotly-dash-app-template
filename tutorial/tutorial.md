...
│   ├── app.py
│   └── index.py
├── poetry.lock             # this folder contains the versions of the packages
├── pyproject.toml          # this file orchestrate your project and its dependencies
</pre>
...
### poetry.lock
When you run the command `poetry install` (see next section), two things can happen:
* **Installing and creating the file poetry.lock:** When you call the `install` command for the first time, Poetry creates a `poetry.lock` file. First, Poetry resolves all dependencies from the `pyproject.toml` file and downloads the latest version. Poetry then installs the dependencies and saves the packages with exact versions in the file `poetry.lock`. You have to commit the `poetry.lock` file to your version control (VC) so that all people in the project work with the same versions of dependencies.

* **Installing and using the file poetry.lock:** If a `poetry.lock` file already exists, then someone else in the project has already run `poetry install` and committed it. In this tutorial, we provide actual `poetry.lock` file. Poetry then installs exactly the versions from the `poetry.lock` file so that the package versions are consistent for everyone. The package versions are not always the last available version so unexpected changes in dependencies do not break the project.

**Note:** It is essential to commit the `poetry.lock` file in VC that ensures that all project members work with the same package versions.
...
version = "1.0.0"
description = ""
authors = ["Tinz, Twins <info@tinztwinshub.com>"]
readme = "README.md"
packages = [{include = "dash-app"}]
...
For more information about Poetry, we recommend Poetry's well-structured [documentation](https://python-poetry.org/docs/).     
...
Now you can build the app with the following command (Make sure you are in the directory with the Dockerfile.):
```bash
